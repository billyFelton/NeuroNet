...
class ClaudeWorker(BaseService):
    ...
    def handle_message(self, envelope: MessageEnvelope) -> None:
        ...
        local_tz = os.environ.get("KEVIN_TIMEZONE", "America/New_York")
        try:
            from zoneinfo import ZoneInfo
            local_now = now.astimezone(ZoneInfo(local_tz))
            time_str = local_now.strftime("%A, %B %d, %Y at %I:%M %p %Z")
        except Exception:
        ...
        logger.info("Publishing AI response to reply_key=%s, corr=%s", reply_key, reply.correlation_id)
        self.rmq.publish(reply_key, reply)
        logger.info("AI response published successfully")
        ...
        # Check if Kevin wants to DM someone
        try:
            self._check_slack_dm(response_text, envelope)
            if debug_level != "none" and "**DM To:**" in response_text:
                debug_trace.append(":speech_balloon: *Action:* Slack DM dispatched")
        except Exception as e:
...
Return ONLY valid JSON, no markdown."""
    ...
    def _check_slack_dm(self, response_text: str, envelope: MessageEnvelope) -> None:
        """
        Parse Kevin's response for a Slack DM request.

        Expected format:
        **DM To:** user@example.com
        **DM Message:**
        ...
        recipient_email = to_match.group(1).strip().rstrip(".")
        ...
        if not msg_match:
            # Try simpler pattern — everything after DM Message:
            msg_match = re.search(
                r'\*\*DM\s+Message:\*\*\s*(.*)',
                response_text,
                re.DOTALL,
        ...
        if not msg_match:
            logger.debug("Found DM To: but no DM Message: in response")
            return

        dm_text = msg_match.group(1).strip()
        if not dm_text:
            return

        logger.info("Kevin wants to DM %s: %.80s", recipient_email, dm_text)

        # Publish to Slack connector
        dm_msg = envelope.create_child(
            source=self.service_name,
            message_type="slack.command.dm",
            payload={
                "recipient_email": recipient_email,
                "text": dm_text,
            },
        )
        self.rmq.publish("slack.command.dm", dm_msg)
        logger.info("Published Slack DM command for %s", recipient_email)
